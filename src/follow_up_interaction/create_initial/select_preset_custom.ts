...
async function select_create_initial_preset_custom(interaction: StringSelectMenuInteraction): Promise<void> {
    ...
    if (settings !== null) {
        //Update first
        const [affectedCount] = await GAME_CREATE.update({ is_preset: new_is_preset }, { where: { clientId: interaction.user.id } });
        if (affectedCount <= 0) {
    ...
    async function message_timeout(update_msg: Message): Promise<void> {
        ...
        if (settings !== null) {
            try {
                ...
                if (display_arr.length !== 1) {
                    console.error('DSPY error at ./utility/create_initial/select_preset_custom.js, no6');
                    await update_msg.edit({content: config['display_error'], components: []});
                    return;
                }
                console.error(`D3 error at ./utility/create_initial/select_preset_custom.js, no7`);
