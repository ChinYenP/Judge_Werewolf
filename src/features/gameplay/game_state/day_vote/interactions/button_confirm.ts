...
import { buttonGameplayDayVoteConfirm } from "../../../../../global/types/interaction_states.js";
...
const button_day_vote_confirm_interaction: InteractionModule<ButtonInteraction, buttonGameplayDayVoteConfirm> = {
    interaction_name: 'button_gameplay_day_vote_confirm',
    states: {
        next_state: {
            execute: async function (interaction: ButtonInteraction): Promise<void> {
                ...
                if (lynched_index === null) {
                    ...
                    if (new_players_info[lynched_index] === undefined) {
                        console.error('The lynched player must exist.');
                        const errorEmbed: EmbedBuilder = await ui_error_fatal(clientId, 'U');
                        await interaction.reply({embeds: [errorEmbed], components: []});
                        return;
                    }
                    ...
                    if (new_players_info[lynched_index].extra_info.role_id === 'G01') {
                        if (new_players_info[lynched_index].extra_info.target !== null && new_players_info[lynched_index].extra_info.target !== lynched_index) {
                            const hunter_target: number = new_players_info[lynched_index].extra_info.target;
                            ...
                            new_turn_order.push('hunter_day');
                        }
                    }
                }
                new_turn_order.push('night');
                ...
                //Create an embed for this day vote result.
                const dayVoteResultEmbed: EmbedBuilder = new EmbedBuilder()
                    .setColor(embed_hex_color)
                    .setTitle(last_day_vote_title ?? display_error_str)
                    .setDescription(day_vote_result_desc)
                    .setTimestamp()
                ...
                const gameUIObj: {error: true, code: t_error_code} |
                    ...
                    = await game_next_state(clientId, dayVoteResultEmbed);
                if (gameUIObj.error) {
                ...
                if (gameUIObj.prevStateEmbed === null) {
                    console.error('Logically, prevState should exist since it is the dayVoteResultEmbed.');
                    const errorEmbed: EmbedBuilder = await ui_error_fatal(clientId, 'U');
                    await interaction.reply({embeds: [errorEmbed], components: []});
                    return;
                }
...
export default button_day_vote_confirm_interaction;