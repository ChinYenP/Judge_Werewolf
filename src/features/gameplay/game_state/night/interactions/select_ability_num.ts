...
import { selectGameplayNightAbilityNum } from "../../../../../global/types/interaction_states.js";
...
const select_night_ability_num_interaction: InteractionModule<StringSelectMenuInteraction, selectGameplayNightAbilityNum> = {
    interaction_name: 'select_gameplay_night_ability_num',
    states: {
        night: {
            execute: async function (interaction: StringSelectMenuInteraction): Promise<void> {
                ...
                if (interaction.values[0] === undefined || game_match.status.status !== 'night') {
                ...
                new_status.selecting.ability = Number(interaction.values[0]);
    ...
    entry: async function(interaction: StringSelectMenuInteraction): Promise<void> {
        console.log('interaction run: select_gameplay_night_ability_num');
        const clientId: string = interaction.user.id;
...
export default select_night_ability_num_interaction;