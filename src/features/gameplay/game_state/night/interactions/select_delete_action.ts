...
import { selectGameplayNightDeleteAction } from "../../../../../global/types/interaction_states.js";
...
const select_night_delete_interaction: InteractionModule<StringSelectMenuInteraction, selectGameplayNightDeleteAction> = {
    interaction_name: 'select_gameplay_night_delete_action',
    states: {
        night: {
            execute: async function (interaction: StringSelectMenuInteraction): Promise<void> {
                ...
                if (interaction.values[0] === undefined || game_match.status.status !== 'night' || game_match.status.actions[Number(interaction.values[0])] === undefined) {
                ...
                new_status.actions = game_match.status.actions.toSpliced(Number(interaction.values[0]), 1);
    ...
    entry: async function(interaction: StringSelectMenuInteraction): Promise<void> {
        console.log('interaction run: select_gameplay_night_delete_action');
        const clientId: string = interaction.user.id;
...
export default select_night_delete_interaction;