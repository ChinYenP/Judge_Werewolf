...
import { t_languages } from '../../../global/types/list_str.js';
    ...
    states: {
        user_settings: {
            execute: async function (interaction: StringSelectMenuInteraction): Promise<void> {
                ...
                const update_msg_resource: InteractionCallbackResource = (await interaction.update({ embeds: [userEmbed, serverEmbed], components: rowLang, withResponse: true })).resource!;
                const update_msg: Message = update_msg_resource.message!;
                await timeout_set('user_settings', update_msg.id, clientId, this.timeout_sec, this.timeout_execute, update_msg, [userEmbed, serverEmbed]);
            },
            timeout: true,
