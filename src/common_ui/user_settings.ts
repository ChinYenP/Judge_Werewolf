...
async function ui_user_settings(clientId: string, time_sec: number): Promise<[ActionRowBuilder<StringSelectMenuBuilder>, EmbedBuilder, EmbedBuilder, EmbedBuilder]> {
    ...
    const [ user_title_text, user_lang_title_text, user_lang_description_text, server_title_text, server_prefix_title_text,
        server_prefix_description_text, timeout_text, lang_placeholder_text ]: string[]
        = await get_display_text(['settings.embed.user_settings.title', 'settings.embed.user_settings.list.language.title',
            'settings.embed.user_settings.list.language.description', 'settings.embed.server_settings.title',
    ...
    const timeoutEmbed: EmbedBuilder = new EmbedBuilder()
        ...
        .addFields(
            {
                name: '\u200b',
                value: `${timeout_text ?? config['display_error']}${time_sec}s`
            }
        );

    return ([rowLang, userEmbed, serverEmbed, timeoutEmbed]);
}
