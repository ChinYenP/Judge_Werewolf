...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (args.length == 1) {
            //Too less arguments
            let less_args_text = await get_display_error("less_args", message.author.id);
        ...
        if (args.length > 2) {
            //Too much arguments
            let much_args_text = await get_display_error("much_args", message.author.id);
        ...
        if (args.length == 2 && !(message.member.permissionsIn(message.channel).has("Administrator"))) {
            ...
            let no_administrator_text = await get_display_error("not_administrator", message.author.id);
        ...
        if (args.length == 2 && message.member.permissionsIn(message.channel).has("Administrator")) {
            if (args[0] == "prefix") {
                ...
                let invalid_prefix_text = await get_display_error("settings_prefix_invalid", message.author.id);
            ...
            let args_error_text = await get_display_error("wrong_args", message.author.id);
...
async function general_settings(message, language) {
    ...
    let timeout_text = '';
    let time_sec = display_text.general.timeout_sec.settings.general;
    let allowed_symbol_text = process.env.ALLOWED_PREFIX_CHARACTERS;
    switch(language) {
        case "eng":
...
async function prefix_settings(message, args, language) {
    ...
    let button_no_text = '';
    let timeout_text = '';
    let time_sec = display_text.general.timeout_sec.settings.server.prefix;
    switch(language) {
        case "eng":
    ...
    async function message_timeout(bot_reply) {
        let timeout_content = confirmation_text + args[1] + "\n\n" + timeout_text;
        await bot_reply.edit({ content: timeout_content, components: [] });
    };
